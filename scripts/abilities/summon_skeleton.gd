...
func execute(options: Dictionary = {}) -> bool:
	...
	if ray_result != {}:
		...
		var skeleton_instance: Skeleton = options["ability_data"].creature_resource.creature.instantiate();
		skeleton_instance.data = options["ability_data"].creature_resource;
